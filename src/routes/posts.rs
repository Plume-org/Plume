            ...
            .expect("posts::update: is author in error")
        {
            ...
            let (content, mentions, hashtags) = utils::md_to_html(
                ...
                &Instance::get_local(&conn)
                    ...
                    .public_domain,
                false,
                Some(Media::get_media_processor(&conn, b.list_authors(&conn).expect("Could not get author list").iter().collect())),
            );
    ...
    if errors.is_empty() {
        ...
        let (content, mentions, hashtags) = utils::md_to_html(
            ...
            &Instance::get_local(&conn)
                ...
                .public_domain,
            false,
            Some(Media::get_media_processor(&conn, blog.list_authors(&conn).expect("Could not get author list").iter().collect())),
        );
